...
class AktiverMeldingService(
    ...
    fun start() {
        val aktiverMeldinger =
            filtrerBortPlanlagteMeldingerForSammeFnr(
                    database.hentPlanlagteMeldingerSomSkalAktiveres(OffsetDateTime.now())
                )
...
// Det feiler i Arena hvis vi sender f.eks. 4-ukersmelding og 8-ukersmelding for samme fnr for tett
// på hverandre
fun filtrerBortPlanlagteMeldingerForSammeFnr(
    planlagteMeldinger: List<PlanlagtMeldingDbModel>
): List<PlanlagtMeldingDbModel> {
    return planlagteMeldinger
        .groupBy { it.fnr }
        .entries
        .mapNotNull { it.value.minByOrNull { planlagtMelding -> planlagtMelding.sendes } }
}
